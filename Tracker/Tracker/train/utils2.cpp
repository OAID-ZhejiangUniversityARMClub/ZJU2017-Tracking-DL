...
void trainNet()
{
	...
	ReadSolverParamsFromTextFileOrDie("squeezehalfSolver.prototxt", &solverParam);
	ReadNetParamsFromBinaryFileOrDie("train_iter_170000.caffemodel", &caffenetParam);
...
void testNet(string modelPath)
{
	...
	Net<float> net("goturnTrain.prototxt", TEST);
...
void buildDB()
{
	...
	for (int i = 1; i <= 10 ; i++)
	{
		string fileName = "/home/hooks/ALOV_h5/trainDataset_" + to_string(i) + ".h5";
