...
int main (int argc, char *argv[]) {
	...
	gettimeofday(&tstart, NULL);
	    ...
	    //#ifdef CPU_ONLY  
            //std::cout<<"test111"<<endl;
	    //#endif
	    /*
	    if(bbox_estimate.x2_-bbox_estimate.x1_>300)
	    {
		if(flag ==0)
		{
		    bbox_estimate_re = bbox_estimate;
		    flag = 1;
		}
		 bbox_estimate = bbox_estimate_re;
	    ...
	    */
 	    ...
 	    //BoundingBox bbbox_gt(region);_
	    //rectangle(org, Point(bbbox_gt.x1_, bbbox_gt.y1_), Point(bbbox_gt.x2_, bbbox_gt.y2_), Scalar(0,0,255), 1, 8, 0);
	...
	//unsigned long diff_time = 1000000 * (tend.tv_sec - tstart.tv_sec) + tend.tv_usec - tstart.tv_usec;
	//std::cout<<"Total time is: "<<diff_time/1000<<"ms"<<std::endl;

	j++;
        diff_time += 1000000 * (tend.tv_sec - tstart.tv_sec) + tend.tv_usec - tstart.tv_usec; 
	if(j==10)
	{
           std::cout << "Total time is " << diff_time/10000 << "ms" << std::endl;
           diff_time = 0;
           j = 0;
        }
