	...
	#else
	  mean_ = cv::Mat(input_geometry_, CV_32FC3, cv::Scalar(0, 0, 0));
	#endif
	//LinF
	}
	...
	void Regressor::Estimate(const cv::Mat& image, const cv::Mat& target, std::vector<float>* output) {
	  ...
	  struct timeval tvstart, tvend; 
          ...
          gettimeofday(&tvstart, NULL);//LinF

          net_->ForwardPrefilled();
  
          gettimeofday(&tvend, NULL);//LinF
	  static int i = 0;
	  i++;
          diff += 1000000 * (tvend.tv_sec - tvstart.tv_sec) + tvend.tv_usec - tvstart.tv_usec; 
	  if(i==10)
	  {
             std::cout << "Runing time: " << diff/10000 << "ms" << std::endl;
	     diff = 0;
	     i = 0;
	  }
	  
          //std::cout << "Runing time: " << diff/1000 << "ms" << std::endl;
...
//LinF
#ifdef TEST_TRAIN_TOOLKIT
  sample_resized = sample_resized - 128;
#endif
//LinF
  cv::Mat sample_float;
  if (num_channels_ == 3)
